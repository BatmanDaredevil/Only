...
public class ReusableLibrary2 {
	...
	public Response getCall(String endpoint,String param1, String value1) throws Exception
	{
		Response response = RestAssured.given().param(param1, value1).when().get(endpoint).then().log().all().extract().response();
	...
	{
		Assert.assertEquals(jsonObject.getString("[0].book_name"), bookName);
